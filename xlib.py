...
try:
  ...
  print('Error: import from Xlib failed')
  exit(1)
...
class XEventsListener(threading.Thread):
  ...
  def _handle_mouse(self, event, value):
    ...
    if value == 2:
      ...
      self.events.append(XEvent('EV_REL',
          0, XEventsListener._butn_to_code.get(event.detail, 'BTN_%d' % event.detail), value))
    else:
